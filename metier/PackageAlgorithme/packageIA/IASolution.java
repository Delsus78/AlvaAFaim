...
public class IASolution {
    ...
    private Arbre arbre;
    private AstarAlgorithme astar;
    private static IASolution instance; 
    
    /**
     * Constructeur de l'IA (calcul coût) 
     * @param partie partie dans laquelle l'IA va effectuer ses calculs
     */
    private IASolution(Partie partie) {
        String seed = partie.getSeed();
        this.partie = new Partie(seed);
    }
    
    public static IASolution get(Partie partie) {
        if (instance == null) instance = new IASolution(partie);
        else if (partie != instance.partie) instance = new IASolution(partie);
        return instance;
    }
    
    /**
     * arrete le calcul de l'IA
     */
    public void stop() {
        this.astar.setStop(true);
    }
    ...
    public int start() {
        ...
        this.astar = new AstarAlgorithme(arbre, 1, 100, partie);
        astar.Calcul();
